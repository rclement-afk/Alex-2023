...
#define target_theta_90 507500 // BEST LEFT TURN = 104,200 BEST RIGHT TURN = 101, 220
...
void move(l_speed,r_speed){
    move(right_motor,r_speed);
    move(left_motor,l_speed);
}
...
void create_line_follow_back(int dist){
    set_create_distance(0);
    while (get_create_distance() < dist){
        if (get_create_lfcliff_amt() < 1600){
            create_drive_direct(134,200);
...
{
    set_create_total_angle(1); //Clears create angle counter
    create_drive_direct(speed,-speed); //Initializes movement
    while(get_create_total_angle()>-degrees) //Waits until angle turned reaches degrees
    {
        msleep(5);
